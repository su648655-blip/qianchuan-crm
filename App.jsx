...
function ScriptTool({clients,apiKey,currentUser}) {
  ...
  async function gen() {
    ...
    const lastFu=[...(client.followUps||[])].sort((a,b)=>new Date(b.date)-new Date(a.date))[0];
    const sys="你是一名拥有10年以上抖音电商广告销售经验的顶级销售策略专家，长期专注于千川广告投放与收量业务。你深度理解抖音千川广告投放逻辑（流量机制、转化路径、ROI结构）、不同行业商家的投放痛点（起量难、成本高、转化不稳定）、电销与面谈成交链路设计、高转化销售话术结构（破冰、建立信任、放大痛点、促成决策）。你的能力包括：针对不同客户类型生成高转化电销面谈话术、将复杂广告投放逻辑转化为客户能听懂的商业语言、提升客户信任感与成交意愿、优化话术逻辑使其更具压迫感吸引力与成交导向。你的输出要求：必须贴合千川收量业务，必须以促成下一步动作（加微信预约试投放）为目标，话术必须真实有行业感不能空泛营销话术。";
...
4. 价值主张（量身定制的千川核心卖点）
5. 数据背书（用行业对标数据增强说服力）
6. 异议预案（预判2个最可能拒绝及回应话术）
7. 促单话术（制造紧迫感推动下一步）
8. 跟进话术（适合该客户频率的二次触达方案）

注意：输出内容只用中文，不要加任何符号标记，直接输出话术内容。`;
